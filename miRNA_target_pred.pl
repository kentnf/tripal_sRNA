...
# 20170214 modify this program for tripal
# 20151003 check_blastall(), do not need put blastall in bin  
...
=head2 
 query_seq_from_file
 put fasta sequence to hash: %qseq_hash, 
 	key: seqID ; value: sequence 
 put sequence id to array: @description_list
=cut
sub query_seq_from_file ($) {
    ...
    return (\%qseq_hash, \@description_list);
}
...
sub print_tab {
	my ($schash, $resulthash, $threshold) = @_;

	# the order of cutoff
	die "[ERR]number of threshold\n" unless scalar(@$threshold) == 4;
	my $th_s = $$threshold[0]; # score
	my $th_m = $$threshold[1]; # mismatch
	my $th_w = $$threshold[2]; # wobble
	my $th_i = $$threshold[3]; # indel
	...
	foreach my $value (sort {$schash{$a} <=> $schash{$b}} keys %schash) {
	  ...
	  if ( (($th_s >= $sc) and ($sc <= 6.0) and $sc > 0) and (($th_m >= $mc) and ($mc <= 8)) and (($th_w >= $wc) and ($wc <= 5)) and (($th_i >= $ic) and ($ic <= 2)) )
	  {
		...
		$summary.= "$sRNA_ID\t$unigene_ID\t$target_start\t$lp\t$align_score\t$align_query\t$align_string\t$align_hit\t$seq_strand\t$mc\t$wc\t$ic\n";
		$i++;
      }
	}
	return $summary;
...
sub filter_output_result
{
	...
	# for strand + the sRNA is 3-5, and mRNA is 5-3 
	my $title = "sRNAID\tmRNAID\tstart\tend\tscore\tmRNA\tsRNA\talign\tstrand\tmismatch\twobble\tindel\n";
	$result_tab_filtered = $title.$result_tab_filtered;
...
sub run_target_score 
{
	...
	my $usage = qq'
USAGE: $0 [options] sRNA_seq  reference_seq  output(options)

	-q	query_type (0 miRNA, 1 mRNA)
		the reference_seq will be mRNA for type 0, miRNA for type 1
	-s	score (0-4)
	-m	mismatch (0-4)
	-w	G:U Wobble (0-5)
	-i	indel (0-1)
	-r	strand (both, forward, reverse: default: both  )
	-x	link out to miRNA or gene feature
...
';
	...
	# === check parameters === 
	my $th_score = 3;				# threshold of score (0-4)
	if (defined $$options{'s'} && $$options{'s'} >= 0 && $$options{'s'} <= 4) {
		$th_score = $$options{'s'};
	}

	my $th_mismatch = 3;			# threshold of mismatch (0-4)
	if (defined $$options{'m'} && $$options{'m'} >= 0 && $$options{'m'} <= 4) {
		$th_mismatch = $$options{'m'};
	}

    my $th_wobble = 2;				# cutoff of target score (0-10)
    if (defined $$options{'w'} && $$options{'w'} >= 0 && $$options{'w'} <= 5) {
        $th_wobble = $$options{'w'};
    }

    my $th_indel = 0;				# cutoff of indel (0-1)
    if (defined $$options{'i'} && $$options{'i'} >= 0 && $$options{'i'} <= 1) {
        $th_indel = $$options{'i'};
    }

    my @threshold = ($th_score, $th_mismatch, $th_wobble, $th_indel);

	my $search_direction = "both";  # Direction  (forward, reverse, both)
	if (defined $$options{'r'}) {
		if ($$options{'r'} =~ m/^(both|forward|reverse)$/ig) {
			$search_direction = lc($$options{'r'});
		}
	}

	my $seq_type = 0;				# type of query sequence (smRNA: 0, mRNA: 1)
	$seq_type = $$options{'q'} if (defined $$options{'q'} && $$options{'q'} eq '1');

	# === below parameters are fixed in current program ===
	my $sc_cutoff = $th_score;			# old score cutoff in program, make it same with th_score
	...
	my ($seq_querys, $description_list) = query_seq_from_file($query_file);	# query seq to hash
	my %seq_querys = %$seq_querys;
	my @description_list = @$description_list;
	...
	foreach my $description (@description_list) {
		...
		# let form validate to check the input sequence before analysis	
		...
		$result_tab = print_tab(\%schash, \%resulthash, \@threshold);
